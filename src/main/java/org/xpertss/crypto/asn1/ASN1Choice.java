...
/**
 * This type represents the ASN.1 CHOICE type as specified in
 * ITU-T Recommendation X.680. On decoding, the decoder must
 * be able to decide umambiguously which alternative choice it
 * has to decode. For this reason all elements in a CHOICE
 * type must have distinctive tags.<p>
 *
 * This class does not enforce the distinctive tag rule.
 * Instead, the alternative with the first matching tag
 * should be chosen by decoders. The application that
 * builds the CHOICE type must take care not to produce
 * ambiguous sets of alternatives.<p>
 *
 * This class distinguishes alternative choices and an
 * inner type. Upon decoding, the inner type is selected
 * from the list of choices based on the identifier octets
 * encountered in the encoded stream. This type is then
 * {@link #setInnerType set as the inner type} of this
 * instance. Unless an inner type is set (either explicitly
 * or by means of decoding) the state of the choice is
 * undefined.<p>
 *
 * This instance always mimicks its inner type. The methods
 * {@link ASN1Type#getTag getTag}, {@link ASN1Type#getTagClass
 * getTagClass}, {@link ASN1Type#getValue getValue} all return
 * the appropriate results of the corresponding method of the
 * inner type. On encoding an instance of this class the
 * inner type is encoded.<p>
 *
 * No nested CHOICE classes are supported. In principle
 * this is easily supported but it is not good style to
...
public class ASN1Choice extends ASN1AbstractType {
   ...
   /**
    * Adds the given type as an alternative choice to the
    * collection of choices. The caller has to take care
    * that no ambiguous choices are added. Each added
    * type must have a distinctive tag.
    * <p>
    * CHOICE elements must neither be OPTIONAL nor
    * tagged IMPLICIT. For safety, this method calls
    * {@link ASN1Type#setOptional setOptional}(false) and
    * {@link ASN1Type#setExplicit setExplicit}(true)
    * on the given type. Callers must not alter this
    * setting after adding a type to this choice.
   ...
   /**
    * Returns the choice with the given tag and tagclass
    * if it exists, otherwise <code>null</code> is returned.
    * This method is called by the decoder in order to
    * determine the appropriate type to decode. The returned
    * type is set up as the inner type by the decoder.
    *
   ...
   /**
    * Trims the internal list of choices to the actual
   ...
   /**
    * Clears the internal list of choices. The inner type
    * remains unaffected if it is already set.
    *
    * number of choices stored in it.
    */
   public void clear()
   ...
   /**
    * Sets the tagging of the inner type as either EXPLICIT or
    ...
    *
    * @return <code>true</code> if the inner type is tagged
    *   EXPLICIT and <code>false</code> if it is tagged IMPLICIT.
    * @exception IllegalStateException if the inner type
    *   is not set.
    */
   ...
   public ASN1Type copy()
   {
      try { 
         ...
         for(int i = 0; i < choices.size(); i++) {
            ASN1Type e = (ASN1Type) choices.get(i);
            if(e != null) {
