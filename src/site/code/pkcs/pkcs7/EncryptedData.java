...
public class EncryptedData extends ASN1Sequence implements ASN1RegisteredType {
   ...
   /**
    * Creates an instance and initialises it with the given
    * key, algorithm, and parameters. The parameters can be
    * <code>null</code> if none should be used.
    *
    * @param bea The bulk encryption algorithm name.
    * @param bek The secret key to use.
    * @param params The algorithm parameters or <code>null
    *   </code> if none are present.
    * @exception InvalidAlgorithmParameterException if there
    *   is a problem with the parameters.
    * @exception NullPointerException if <code>bea</code> or
    *   <code>bek</code> are <code>null</code>.
    */
   ...
   /**
    * Initialises the underlying {@link EncryptedContentInfo
    * EncryptedContentInfo} with the given bulk encryption key.
    *
    * @param key The BEK to use for encrypting or decrypting.
    * @exception CryptoException if a problem with
    *   some of the involved provider engines is detected.
    */
